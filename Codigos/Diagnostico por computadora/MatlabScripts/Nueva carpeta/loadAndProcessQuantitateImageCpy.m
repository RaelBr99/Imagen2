...
fcenter = imcrop(f,[32 32 (448*sz(2)/sz(1)-64) 383 ]);

%f=histeq(f,128);

for channel = 1:3
    f(:,:,channel) = f(:,:,channel)-min(min(fcenter(:,:,channel)));
    f(:,:,channel) = f(:,:,channel)./max(max(fcenter(:,:,channel)));
end

minImage = min(f,[],3);
maxImage = max(f,[],3);
medianImage = median(f,3);

f(:,:,1) = maxImage-medianImage;
f(:,:,2) = medianImage;
f(:,:,3) = maxImage-minImage;
...
%minImage = min(f,[],3);
%maskthr = max([0.25,0.75*graythresh(minImage)]);
maskthr = 0.75*median(minImage,'all');
...
templateRadius =  sum(sum(finalmask))/4;
...
controlmask = controlmask & not(finalmask);
...
%    imshow(LesionData,[]);
    ...
    %%%%%%%%%%%%%%%% Temperature GLCM %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    glcm_1 = graycomatrix(LesionData(finalmask),'NumLevels',numGLCMBins,'GrayLimits',[0.0,1.00],'Offset',[0 2; -2 2; -2 0; -2 -2],'Symmetric',true);
    glcm_2 = graycomatrix(LesionData(finalmask),'NumLevels',numGLCMBins,'GrayLimits',[0.0,1.00],'Offset',[0 4; -3 3; -4 0; -3 -3],'Symmetric',true);
    glcm_3 = graycomatrix(LesionData(finalmask),'NumLevels',numGLCMBins,'GrayLimits',[0.0,1.00],'Offset',[0 8; -6 6; -8 0; -6 -6],'Symmetric',true);
    glcm_4 = graycomatrix(LesionData(finalmask),'NumLevels',numGLCMBins,'GrayLimits',[0.0,1.00],'Offset',[0 16; -11 11; -16 0; -11 -11],'Symmetric',true);
    glcm_1 = sum(glcm_1,3);
    glcm_2 = sum(glcm_2,3);
    glcm_3 = sum(glcm_3,3);
    glcm_4 = sum(glcm_4,3);
...
for channel = 1:3
    ...
    ControlData = controlimage(:,:,channel) + not(controlmask);
%    imshow(ControlData,[]);
    ...
    %%%%%%%%%%%%%%%%  GLCM %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    glcm_1 = graycomatrix(ControlData(controlmask),'NumLevels',numGLCMBins,'GrayLimits',[0.0,1.00],'Offset',[0 2; -2 2; -2 0; -2 -2],'Symmetric',true);
    glcm_2 = graycomatrix(ControlData(controlmask),'NumLevels',numGLCMBins,'GrayLimits',[0.0,1.00],'Offset',[0 4; -3 3; -4 0; -3 -3],'Symmetric',true);
    glcm_3 = graycomatrix(ControlData(controlmask),'NumLevels',numGLCMBins,'GrayLimits',[0.0,1.00],'Offset',[0 8; -6 6; -8 0; -6 -6],'Symmetric',true);
    glcm_4 = graycomatrix(ControlData(controlmask),'NumLevels',numGLCMBins,'GrayLimits',[0.0,1.00],'Offset',[0 16; -11 11; -16 0; -11 -11],'Symmetric',true);
    glcm_1 = sum(glcm_1,3);
    glcm_2 = sum(glcm_2,3);
    glcm_3 = sum(glcm_3,3);
    glcm_4 = sum(glcm_4,3);
